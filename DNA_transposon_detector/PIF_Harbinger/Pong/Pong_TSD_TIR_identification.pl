...
LOOP:foreach $key(keys %gSeq)
{
    ...
    while ($gSeq{$key}=~/[ATCG]{6}[GC]T[AC]CAA/g)
    {
        push @tir_F,pos($gSeq{$key});
    }
   

    while ($gSeq{$key}=~/TTG[TG]A[GC][ATCG]{6}/g)
    {
    ...
    open (SEQ,">>Pong.identifed.fasta") or die $!;
        ...
        foreach my $pos2(@tir_R)
        {
            ...
            if($pos1<10000 &&  $pos2>10000+$match_l)
            {
                ...
                if($#match>=12 && $pattern==1)
                {
                    ...
                    print SEQ ">$key","_",$pos1-12,"_",$pos2,"\n";
                    print SEQ $seq,"\n";
                    print ALIGN "TSD: ","\t",$tsd_F,"\t",$tsd_R,"\n";
                    print ALIGN "$key","\t",$pos1-12,"\t",$pos2,"\n";
    ...
    @tir_F=();
    @tir_R=();
    while ($gSeq{$key}=~/GAGCA[TC]/g)
    {
        push @tir_F,pos($gSeq{$key});
    }
   

    while ($gSeq{$key}=~/[AG]TGCTC/g)
    {
        push @tir_R,pos($gSeq{$key});
    }
    ...
    foreach my $pos1(@tir_F)
    {
        foreach my $pos2(@tir_R)
        {
            ...
            if($pos1<10000 &&  $pos2>10000+$match_l)
            {
                my $tsd_F=substr($gSeq{$key},$pos1-9,3);
                my $tsd_R=substr($gSeq{$key},$pos2,3);
                my $tir_F=substr($gSeq{$key},$pos1-6,20);
                my $tir_R=substr($gSeq{$key},$pos2-20,20);
                $tir_R=reverse $tir_R;
                $tir_R=~tr/ATCG/TAGC/;
                ...
                {
                    my $seq=substr($gSeq{$key},$pos1-12,$pos2-$pos1+13);
                    print SEQ ">$key","_",$pos1-6,"_",$pos2,"\n";
                    print SEQ $seq,"\n";
                    print ALIGN "TSD: ","\t",$tsd_F,"\t",$tsd_R,"\n";
                    print ALIGN "$key","\t",$pos1-6,"\t",$pos2,"\n";
                    print ALIGN "$tir_F\t","$tir_R\n";
                    print ALIGN @align1;
                }
            }
